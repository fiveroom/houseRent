				...
				<ScrollBar class="cenchat-scro">
					<ul v-if="arrAdmin.length">
						<li
							...
							@click="currAdmin=item"
						>
							<div class="cen-admin__l">
								<img :src="item.Admin_avaterPath || defCover" alt />
							</div>
							<el-badge class="cen-admin__num" :max="99" :value="200">
		...
		methods: {
			...
			connectAdin() {
				...
				this.webK.message(async data => {
					if (data.nickname != this.userName) {
						let adminData = null;
						await queryAdmin(data.nickname).then(res => {
						...
						if (
							!this.arrAdmin.some(
								item => item.Admin_realName == data.nickname
							)
						) {
							this.arrAdmin.push(adminData);
						}
						for (let i = 0; i < this.arrAdmin.length; i++) {
							if (this.arrAdmin[i].Admin_realName == data.nickname) {
								this.arrAdmin[i].lastMsg = data.content;
							}
						}
						this.$nextTick(() => {
				...
				if (this.userMsg.length && this.currAdmin) {
					...
					this.webK.send(
						JSON.stringify({
							...
							nickname2: this.currAdmin.Admin_realName
						})
					);
			...
			addReciveMsg(data, adminId) {
				if (data.isSelf) {
					...
					console.log(`admin${adminId}`, this.$refs[refName]);
					if (Array.isArray(this.$refs[refName])) {
						this.$refs[refName][0].appendChild(div);
					} else {
						this.$refs[refName].appendChild(div);
