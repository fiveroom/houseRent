...
<script>
	...
	import { queryMyAdmin, queryOwnCollect, queryBespeak } from "@/api/user";
	...
	export default {
		...
		methods: {
			...
			butUserMsg(htmlData, index) {
				if (this.userMsg.length || htmlData) {
					...
					if (htmlData) {
						...
						console.log(this.houseData[index]);
					} else {
						sendContent = {
							type: "text",
							data: this.userMsg
			...
			// 查询管理员
			queryAdmin() {
				queryMyAdmin({ admin_name: "admin_all" }).then(data => {
